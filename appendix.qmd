...
parse_price <- function(c) {
    c |> gsub(pattern = "\\D+", replacement = "") |> parse_number()
}
```
...
add_car <- function(id) {
    ...
    ad_df <- tibble(
        Id = ad_data$ad_id,
        Märke = ifelse(
            "cx_make" %in% ad_data$parameter_groups$parameters[[2]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[2]], 
                id == "cx_make")$value[[1]],
            NA),
        Modell = ifelse(
            "cx_model" %in% ad_data$parameter_groups$parameters[[2]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[2]], 
                id == "cx_model")$value[[1]],
            NA),
        Bränsle = ifelse(
            "fuel" %in% ad_data$parameter_groups$parameters[[1]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[1]], 
                id == "fuel")$value[[1]],
            NA),
        Växellåda = ifelse(
            "gearbox" %in% ad_data$parameter_groups$parameters[[1]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[1]], 
                id == "gearbox")$value[[1]],
            NA),
        Miltal = ifelse(
            "mileage" %in% ad_data$parameter_groups$parameters[[1]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[1]], 
                id == "mileage")$value[[1]],
            NA),
        Modellår = ifelse(
            "regdate" %in% ad_data$parameter_groups$parameters[[1]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[1]], 
                id == "regdate")$value[[1]],
            NA),
        Biltyp = ifelse(
            "car_chassis_type" %in% ad_data$parameter_groups$parameters[[2]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[2]], 
                id == "car_chassis_type")$value[[1]],
            NA),
        Drivning = ifelse(
            "cx_drive_wheels" %in% ad_data$parameter_groups$parameters[[2]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[2]], 
                id == "cx_drive_wheels")$value[[1]],
            NA),
        HK = ifelse(
            "cx_engine_power" %in% ad_data$parameter_groups$parameters[[2]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[2]], 
                id == "cx_engine_power")$value[[1]],
            NA),
        Färg = ifelse(
            "cx_color" %in% ad_data$parameter_groups$parameters[[2]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[2]], 
                id == "cx_color")$value[[1]],
            NA),
        Motorstorlek = ifelse(
            "cx_engine_size" %in% ad_data$parameter_groups$parameters[[2]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[2]], 
                id == "cx_engine_size")$value[[1]],
            NA),
        `Datum i trafik` = ifelse(
            "cx_first_time_in_traffic" %in% ad_data$parameter_groups$parameters[[2]]$id, 
            filter(
                ad_data$parameter_groups$parameters[[2]], 
